...
"""64 位有符号整数下限(-9223372036854775808)。"""
INT64_MAX: int = 0x7FFFFFFFFFFFFFFF
"""64 位有符号整数上限(9223372036854775807)。"""
...
FINS_UNSUPPORTED_AREA_CODE: int = 0x1101
"""结束码:存储区码非法——老固件(CP1E/部分 CS1)不支持 D/EM 区位码时返回。"""
FINS_BIT_FALLBACK_AREAS: Tuple[str, ...] = ("D", "E")
"""位区码不被老固件支持、需回退「字读 + 本地提位」的字存储区(DM/EM)。"""
FINS_END_CODE_HINT: Dict[int, str] = {
    0x0101: "检查本机节点/网络号是否与 PLC 同网(local_node、source_network)",
    0x0105: "检查节点号范围(以太网 1~254)",
    0x0106: "节点号重复:显式指定不同的 source_node",
    0x0201: "检查 destination_network/destination_node(跨网需显式网络号并配好路由表)",
    0x0202: "检查 destination_node 是否为目标 PLC 节点号(通常 = PLC IP 末段)",
    0x0501: "目标网络/节点未登记到路由表",
    0x0502: "源/目标/中继节点未设置路由表",
    0x0503: "路由表内容有误",
    0x0504: "跨网层数超过 3 层,需精简网络或改路由表",
    0x1005: "命令头/路由表错误(检查中继表与本地网络表)",
}
"""常见配置类结束码 → 现场排查提示(拼在结束码文本后;W342 5-1-3)。"""
