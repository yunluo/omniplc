...
- **§2.4.1/2.4.3 AB 0x0A 字节预算(P1)**:`read_batch` 改为按 (条数 ≤32, 估算字节 ≤480B) 双约束自动拆分事务,长标签名不再触顶未连接缓冲;条数硬上限取消。
...
### 4.1 CI 仅 Python 3.12,声明支持 3.7.9 — **P1(实锤)**
- `pyproject.toml:6` + `.github/workflows/ci.yml:28-30`
- 复核确认:CI 注释自认"py3.7 真兼容由本地门禁兜底";且 uv 托管下载不支持 3.7(`ci.yml:28-29`),直接加 3.7 腿有工具链障碍。
- **修复**:二选一——CI 改用 `actions/setup-python` 装 3.7 跑 pytest 腿;或 `requires-python=">=3.9"` 明示断代并改 CHANGELOG。维持现状(声明 3.7 却不验证)最差。
