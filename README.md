...
rtu.inter_frame_delay = 0.004  # 帧间 3.5c 静默(秒);默认 0,高波特率+密集轮询可设
```
...
- **取消是真中断**:`asyncio.wait_for` 超时会取消事务;若请求已发出,连接按
  ...
  则保持连接。
- **超时口径与同步层一致**:TCP 读超时按连接死亡拆连;**UDP 接收**超时不拆连
  (数据报整收,无残渣)、**UDP 发送**超时(本地缓冲打满)按 OSError 语义拆连
...
按版本号降序的完整变更日志已迁出至 [`CHANGELOG.md`](CHANGELOG.md)(从 v0.42.1 到 v0.1 的详细说明);当前发布版本以 Git 标签为准(`git tag -l 'v*'`)。
