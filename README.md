...
- **v0.31.3(当前)**:CI 发布流水线——新增 GitHub Actions 工作流(`.github/workflows/build-release.yml`),打标签推送(`v*`)自动触发:`uv build` 构建 sdist + wheel、挂到对应 GitHub Release,并经**可信发布(OIDC)**发布到 PyPI(工作流内无明文令牌;`fork` 守卫防误发;Gitee Go 流水线为企业付费服务,个人账号不可用故未配置)
